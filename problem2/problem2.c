...
int main(int argc, char* argv[]){
   ...
   rootpid = fork();
   if(rootpid == 0){
    printf("Process Creation and Stopping:\n");

    create_process_tree(root,status);
    printf("All processes die\n");
   }else{
...
void create_process_tree(struct tree_node *root, int status){
    ...
    for (int i =0; i<n; i++){
        ...
        waitpid(resumePID[i], &status, WUNTRACED);
        explain_wait_status(resumePID[i], status);
    }
    ...
    for (int j=0; j<n; j++){
        sleep(1);
        printf("my pid is %d, Process %s is resuming...\n", resumePID[j], root->children_name[j]);
        kill(resumePID[j],SIGCONT);
        waitpid(resumePID[j], &status, WUNTRACED);
        explain_wait_status(resumePID[j], status);
    }

    printf("my pid is %d, Process %s is exiting...\n", getpid(), root->name);
    return;
}
